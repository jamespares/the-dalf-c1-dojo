...
speaking.get('/exams/:id/speaking', authMiddleware(), async (c) => {
  ...
  const recordingAnswer = existingAnswers.find((a) => a.audioKey);
  const hasRecording = !!recordingAnswer;

  return c.html(
    <Layout title={`Oral Production — DALF C1`}>
      <Navbar user={user} />
      <div class="container">
        ...
        <div class="card">
          ...
          {hasRecording && (
            <div class="recording-playback">
              <p style="margin:0 0 var(--space-3);font-weight:500;">Saved recording</p>
              <audio controls src={`/exams/${examId}/audio/${encodeURIComponent(recordingAnswer!.audioKey!)}`} />
            </div>
          )}

          <div id="recordError" class="hidden alert alert-danger" style="margin-top:1rem;"></div>
        </div>
        ...
        <form method="post" action={`/exams/${examId}/speaking/submit?attempt=${attempt.id}`}>
          <button type="submit" id="submitBtn" class="btn btn-success" disabled={!hasRecording}>
            Submit for Marking
          </button>
          {!hasRecording && <p style="color:var(--muted);font-size:0.85rem;margin-top:0.5rem;">Record your exposé before submitting.</p>}
        </form>
